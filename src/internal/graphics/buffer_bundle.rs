...
impl BufferBundle {
    fn create_async(
        ...
        lazy(move || Self::create(&adapter, buffer_len, usage, memory_properties))
    }
    ...
    fn create_staged_bundle<D: Copy + Send + Sync>(
        adapter: Arc<Adapter<backend::Backend>>,
        buffer_size: u64,
        usage: BufferUsage,
        data: Arc<Vec<D>>,
    ) -> impl Future<Item = BufferBundle, Error = Error> + Send {
        BufferBundle::create_async(
            Arc::clone(&adapter),
            buffer_size,
        ...
        .and_then(move |s_buffer| {
            s_buffer
                .bind_data_async(data)
                .and_then(move |bound_buffer| {
                    BufferBundle::create_async(
                        adapter,
                        buffer_size,
                        BufferUsage::TRANSFER_DST | usage,
            ...
            lazy(move || unsafe {
                ...
                let (buff, mut poo) = {
                    let mut pool = device
                        ...
                        .unwrap();
                    let mut buffer: CommandBuffer<backend::Backend, Transfer, OneShot, Primary> =
                ...
                let fut = poll_fn(move || match device.get_fence_status(&fence) {
                    Ok(signaled) => {
                        if signaled {
                            Ok(Async::Ready(()))
                        } else {
    ...
    fn bind_data_async<D: Copy + Send + Sync>(
        self,
        points: Arc<Vec<D>>,
    ) -> impl Future<Item = Self, Error = Error> + Send {
        Box::new(lazy(move || {
