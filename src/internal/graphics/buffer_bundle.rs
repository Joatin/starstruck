...
pub struct BufferBundle<B: Backend, D: Device<B>, I: Instance<Backend=B>, P: BufferBundlePlace, T: Copy + Send + Sync> {
...
impl<B: Backend, D: Device<B>, I: Instance<Backend=B>, T: Copy + Send + Sync> BufferBundle<B, D, I, CPU, T> {
...
impl<B: Backend, D: Device<B>, I: Instance<Backend=B>> BufferBundle<B, D, I, CPU, image::Rgba<u8>> {
    pub fn write_image_data(self, image: RgbaImage, limits: Limits) -> impl Future<Item = Self, Error = Error> + Send {
        lazy(move || {
            info!("Writing data into buffer");
            unsafe {
...
impl<B: Backend, D: Device<B>, I: Instance<Backend=B>, T: Copy + Send + Sync> BufferBundle<B, D, I, GPU, T> {
    pub fn new(
        state: Arc<GraphicsState<B, D, I>>,
        usage: BufferUsage,
        data: Arc<Vec<T>>
        ...
        let bundle_future = Self::create_buffer(Arc::clone(&state), buffer_len, usage | BufferUsage::TRANSFER_DST, Properties::DEVICE_LOCAL);
        let transfer_bundle_future = BufferBundle::<B, D, I, CPU, T>::create_buffer(state, buffer_len, BufferUsage::TRANSFER_SRC, Properties::CPU_VISIBLE);

        bundle_future.join(transfer_bundle_future).and_then(move |(bundle, transfer_bundle)| {
            transfer_bundle.write_data(data)
                .and_then(move |t_bundle| {
                    bundle.import_data_from(t_bundle)
                })
        })
    }
    ...
    fn import_data_from(
        ...
        lazy(move || unsafe {
            ...
            poll_fn(move || unsafe { match dev.get_fence_status(&fe) {
                Ok(signaled) => {
                    if signaled {
                        Ok(Async::Ready(()))
                    } else {
...
impl<B: Backend, D: Device<B>, I: Instance<Backend=B>, P: BufferBundlePlace, T: Copy + Send + Sync> BufferBundle<B, D, I, P, T> {
    ...
    pub fn create_buffer(
        ...
        lazy(move || {
            ...
            unsafe {
                ...
                let memory = state.device().allocate_memory(memory_type_id, requirements.size)?;
...
impl<B: Backend, D: Device<B>, I: Instance<Backend=B>, P: BufferBundlePlace, T: Copy + Send + Sync> Drop for BufferBundle<B, D, I, P, T> {
    fn drop(&mut self) {
        use core::ptr::read;
